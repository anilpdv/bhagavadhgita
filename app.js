...
setInterval(generateKey, 300000);
...
app.get('/chapter/show/:id', function(req, res, next) {
  ...
  axios
    .get('https://bhagavadgita.io/api/v1/chapters/' + id, {
      params: {
        access_token: `${key}`,
      },
    })
...
app.get('/chapter/show/:id/verses', function(req, res, next) {
  let id = req.params.id;
  let url = util.format(
    'https://bhagavadgita.io/api/v1/chapters/%s/verses',
    id,
  );
  axios
    .get(url, {
...
app.listen(process.env.PORT || 3001, function() {
  generateKey();
  console.log('app start and running', '3001');
});
